...
class MySQLPipeline(object):
    ...
    def _conditional_insert(self, tx, item):
        ...
        if result:
            ...
            tx.execute(\
                "insert into product_capture (title,link,name,type,picture,content,price,payLink,setting,tag,status,createTime) "
                "values (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                (item['title'],item['link'],item['name'],1,item['picture'],item['content'],item['price'],item['paylink'],item['setting'],item['tag'],0,
